...
const int degrees[2] = {2, 10};
const int points[3] = {6, 10, 20};
...
std::vector<point>
    getRandomPoints(double (*f) (double), int n) {
        std::vector<point> points;
        for (int i = 0; i < n; ++i) {
            double val = random_n() * (END - START) + START;
            points.push_back(point{val, f(val)});
        }
        return points;
    }
...
int main(){
    ...
    for (int number : points) {
        ...
        std::vector<point> values = getRandomPoints(function, number);
        for (point value : values)
